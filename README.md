...
```
The PHP application should query the NHTSA API and use the data received to respond with precisely the following JSON if there are results:
```
{
 ...
 * `<VEHICLE DESCRIPTION>` is the name of the vehicle model returned from the NHTSA API and is a string In order to get the data to generate your application's response, you will need to call the following NHTSA API endpoint:
```
...
### Requirement 2
The application also responds on an additional endpoint:
```
...
```
Where `<VehicleId>` is one of the vehicle IDs from the initial API response (so 9403, 9408, 9405, 9406 in the case of the 2015 Audi A3). The response from NHTSA looks like this (vehicle has a rating -- you need the value of `OverallRating`):
```json
{
  "Count": 1,
  "Message": "Results returned successfully",
...
```
Here's an example response should return for Requirement 3 when testing with the 2015 Audi A3:
```json
{
  "Count": 4,
  "Results": [
